...
class TimeSeries:
    ...
    def from_xml(cls, xml: ET.Element):
        ...
        values_map = {
            int(point.find("{urn:iec62325.351:tc57wg16:451-3:publicationdocument:7:0}position").text): float(
                point.find("{urn:iec62325.351:tc57wg16:451-3:publicationdocument:7:0}price.amount").text
            )
    ...
    def to_period(self) -> dict[str, float]:
        ...
        return {str(start_time + timedelta(hours=i)): value for i, value in enumerate(self.period)}
...
class EngieIndexMonth:
    ...
    def __post_init__(self):
        values = [value for time_serie in self.timeseries if time_serie.filter(month=self.month, year=self.year) for value in time_serie.period]
...
timeseries = [TimeSeries.from_xml(child) for child in energy_root.iter("{urn:iec62325.351:tc57wg16:451-3:publicationdocument:7:0}TimeSeries")]
